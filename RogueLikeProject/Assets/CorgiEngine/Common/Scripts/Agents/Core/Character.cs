...
{	
	...
	{		
		...
		protected virtual void RotateModel()
		{
			...
			{
				Quaternion targetRotation = Quaternion.Euler(_targetModelRotation);
				CharacterModel.transform.localRotation = Quaternion.RotateTowards(
					CharacterModel.transform.localRotation,
					targetRotation,
					ModelRotationSpeed * Time.deltaTime * 100
				);
			}
			else
			{
		...
		public virtual void Face(FacingDirections facingDirection, bool forceImmediateRotation = false)
		{
			if (!CanFlip)
			{
				return;
			...
			if (facingDirection == FacingDirections.Right)
			{
				...
				{
					Flip(true);
					if (forceImmediateRotation && RotateModelOnDirectionChange)
					{
						_targetModelRotation = Vector3.zero;
						CharacterModel.transform.localRotation = Quaternion.Euler(_targetModelRotation);
					}
				}
			}
			else
