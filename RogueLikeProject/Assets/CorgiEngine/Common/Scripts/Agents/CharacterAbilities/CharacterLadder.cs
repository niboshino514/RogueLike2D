...
{	
	...
	public class CharacterLadder : CharacterAbility 
	{
		...
		public virtual void SetInput(Vector2 ladderInput, float threshold)
		{
			_ladderInput = ladderInput;
		...
		protected virtual void HandleLadderClimbing()
		{
			...
			if (LadderColliding) 
			{
				...
				{
					_ladderInput.x = _horizontalInput;
					_ladderInput.y = _verticalInput;
					_threshold = _inputManager.Threshold.y;
				}
				...
				else
				{
					...
					inputConditionsMet = _ladderInput.y > _threshold;
				}
				...
				if (LowestLadder.LadderPlatform != null)
				{
					...
					if ((_movement.CurrentState != CharacterStates.MovementStates.LadderClimbing) // if the character is climbing
					    ...
					    && (_ladderInput.y < -_threshold) // and is pressing down
		...
		protected virtual void StartClimbing()
		{
			...
			if ((_characterHandleWeapon != null) && (_characterHandleWeapon.isActiveAndEnabled) && (!_characterHandleWeapon.CanShootFromLadders))
			{
		...
		protected virtual void Climbing()
		{
			...
			{
				_controller.SetVerticalForce(_ladderInput.y * LadderClimbingSpeed);
				// we set the climbing speed state.
				CurrentLadderClimbingSpeed = Mathf.Abs(_ladderInput.y ) * transform.up;	
			}
			if (CurrentLadder.LadderType == Ladder.LadderTypes.BiDirectional)
			{
				_controller.SetHorizontalForce(_ladderInput.x * LadderClimbingSpeed);
				_controller.SetVerticalForce(_ladderInput.y * LadderClimbingSpeed);
				CurrentLadderClimbingSpeed = Mathf.Abs(_ladderInput.x ) * transform.right;	
				CurrentLadderClimbingSpeed += Mathf.Abs(_ladderInput.y ) * (Vector2)transform.up;	
			}
		...
		public virtual bool AboveLadderPlatform()
		{
