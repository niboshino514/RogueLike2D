...
{	
	...
	public class CorgiController : CorgiMonoBehaviour 
	{
		...
		protected bool _movingPlatformTest;
		...
		protected virtual void CastRaysBelow()
		{
			...
			if (hitConnected)
			{
				...
				_movingPlatformTest =
					(MovingPlatformMask.MMContains(_belowHitsStorage[smallestDistanceIndex].collider.gameObject.layer) 
					 || MovingOneWayPlatformMask.MMContains(_belowHitsStorage[smallestDistanceIndex].collider.gameObject.layer));
				
				if (_movingPlatformTest && State.IsGrounded)
				{
					_movingPlatform = _belowHitsStorage[smallestDistanceIndex].collider.gameObject.GetComponent<MMPathMovement>();
				}
				else
				{
		...
		protected virtual void CastRaysAbove()
		{
			...
			rayLength += RayExtraLengthVertical;
		...
		protected virtual void OnCorgiColliderHit() 
		{
			...
			{	
				if (Parameters.Physics2DInteraction)
				{
					...
					if (body == null || body.bodyType == RigidbodyType2D.Kinematic || body.bodyType == RigidbodyType2D.Static)
					{
						return;
					}                        
