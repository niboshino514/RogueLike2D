	...
	public class CharacterSwitchManager : CorgiMonoBehaviour
	{
		...
		protected virtual void InstantiateCharacters()
		{
			...
			for (int i = 0; i < CharacterPrefabs.Length; i++)
			{
				...
				if (PersistHealthOverSwitches)
				{
					Health health = newCharacter.gameObject.MMGetComponentNoAlloc<Health>();
					if (health != null)
					{
						health.ResetHealthOnEnable = false;
					}
				}
		...
		protected virtual IEnumerator OperateSwitch()
		{
			...
			if (MaintainPreviousCharacterFacingDirection)
			{
				...
				LevelManager.Instance.Players[PlayerIndex].Face(facingDirection, true);
			}
		}
	}
}