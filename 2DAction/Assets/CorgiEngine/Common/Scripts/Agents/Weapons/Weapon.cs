...
namespace MoreMountains.CorgiEngine
{
	...
	public class Weapon : MMMonoBehaviour
	{
		...
		{
			_cooldownStartAt = float.NegativeInfinity;
		}
		...
		{
			if (WeaponState.CurrentState == WeaponStates.WeaponUse)
			{
		...
		public virtual void InitializeAnimatorParameters()
		{
			for (int i = 0; i < Animators.Count; i++)
			{
				...
				{
					MMAnimatorMirror mirror = Animators[i].gameObject.MMGetOrAddComponent<MMAnimatorMirror>();
