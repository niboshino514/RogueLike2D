...
{	
	...
	public class WeaponAim : CorgiMonoBehaviour 
	{
		...
		{            
			...
			if (_weapon.Owner != null &&
			    _weapon.Owner.ConditionState != null &&
			    (
				    _weapon.Owner.ConditionState.CurrentState == CharacterStates.CharacterConditions.Stunned ||
				    _weapon.Owner.ConditionState.CurrentState == CharacterStates.CharacterConditions.Frozen
			    ))
			{
				return;
			}
